...
type Props = {
  ...
  role?: RoleItem;
  onClose: () => void;
  onSave: (update: { id: string; name: string; description: string }) => void;
};

export function EditRoleModal({ open, role, onClose, onSave }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-[#0f172a]">
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-200 px-5 py-4 dark:border-slate-800">
          ...
          <button
            onClick={() => {
              onSave({ id: role.id, name: name.trim(), description: description.trim() });
              onClose();
            }}
            disabled={!name.trim()}
