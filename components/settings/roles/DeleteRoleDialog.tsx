...
export function DeleteRoleDialog({ open, roleName, onClose, onConfirm }: Props) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-[#0f172a]">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Delete role</h3>
          <button
        ...
        <div className="space-y-3 px-4 py-4 text-sm text-slate-600 dark:text-slate-300">
          <p>
            This will remove the <span className="font-semibold text-slate-900 dark:text-white">{roleName}</span> role.
            Members will lose permissions associated with this role. This action cannot be undone.
          </p>
        </div>
